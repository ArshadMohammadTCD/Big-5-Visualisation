...
def perform_kmeans_clustering(input_file, personality_columns=None, max_clusters=10):
    """
    Perform K-means clustering on age personality scores
    
    Parameters:
    - input_file (str): Path to the CSV file with age personality averages
    ...
    # Save clustered data
    df_with_clusters.to_csv('age_personality_clusters.csv', index=False)
...
def visualize_clusters(clustered_data, personality_columns):
    ...
    # Update layout
    fig.update_layout(
        title='age Personality Clusters',
        plot_bgcolor = 'white',
        paper_bgcolor = 'white'
    )
    
    # Save the plot
    pyo.plot(fig, filename='age_personality_clusters_parallel.html', auto_open=False)